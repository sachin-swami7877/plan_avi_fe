...
const WalletRequests = () => {
  ...
  const fetchRequests = async () => {
    try {
      const res = await adminAPI.getWalletRequests({ type: 'deposit', status: filter !== 'all' ? filter : undefined });
      setRequests(res.data.filter(r => r.type === 'deposit'));
    } catch (error) {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-gray-800 mb-4">Deposit Requests</h1>
        ...
        {requests.length === 0 ? (
          ...
          requests.map((request) => (
            <div key={request._id} className="bg-white rounded-xl overflow-hidden shadow-sm">
              {/* Header */}
              <div className={`p-4 ${
                request.status === 'pending' ? 'bg-yellow-50 border-l-4 border-yellow-500' :
              ...
              <div className="p-4">
                <div className="grid grid-cols-2 gap-3 mb-3">
                  <div className="bg-gray-50 rounded-lg p-3">
                    ...
                    <p className="font-mono font-medium">{request.utrNumber || 'N/A'}</p>
                  </div>
                  <div className="bg-gray-50 rounded-lg p-3">
                ...
                {request.screenshotUrl && (
                  <a
                    href={request.screenshotUrl.startsWith('http') ? request.screenshotUrl : `http://localhost:5001${request.screenshotUrl}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="block bg-primary-50 text-primary-700 text-center py-2 rounded-lg mb-3 font-medium"
                  >
                    View Screenshot
                  </a>
                )}
