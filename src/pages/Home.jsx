  ...
  return (
    <div className="min-h-screen bg-gray-100 pb-24 overflow-x-hidden">
      ...
      <div className="max-w-md mx-auto p-4 w-full min-w-0">
        ...
        {/* User Warning */}
        {userWarning && (
          <div className="mb-4 bg-yellow-50 border border-yellow-300 rounded-xl px-4 py-3 flex items-start gap-2">
            <span className="text-yellow-600 text-lg mt-0.5">&#9888;</span>
            <p className="text-yellow-800 text-sm font-medium">{userWarning}</p>
          </div>
        )}
        ...
        <div className="mb-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            {gameCards.map((game) => (
              ...
              >
                <div className={`h-24 bg-gradient-to-br ${game.gradient} flex items-center justify-center p-4 text-white relative`}>
                  {game.image ? (
                    <>
                      <img
                        src={game.image}
                      ...
                      />
                      <div className="absolute inset-0 hidden items-center justify-center w-14 h-14 flex-shrink-0" style={{ display: 'none' }}>
                        {game.fallbackIcon}
                      </div>
                    </>
                  ) : (
                    <div className="w-14 h-14 flex-shrink-0">{game.fallbackIcon || game.icon}</div>
                  )}
                </div>
