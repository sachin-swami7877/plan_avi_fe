              ...
              <div key={m._id} className={`rounded-xl p-4 shadow-sm border ${st.bg}`}>
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-gray-500 mt-0.5">{formatTime12hr(m.createdAt)} • {m.players?.map((p) => p.userName).join(', ')}</p>
                  </div>
                  <button
                    onClick={() => navigate(`/ludo/match/${m._id}`)}
                    className="flex-shrink-0 text-sm font-semibold text-primary-600 hover:text-primary-800 transition-colors"
                  >
                    View →
                  </button>
                </div>
